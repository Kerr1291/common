...
{ 
    ...
    void Field_LayerSize()
    {
        ...
        Dev.ClampToInt(ref newLayerSize);
    ...
    void SelectMapCell( Rect map_pos, int scale )
    {
        ...
        if( Event.current.type == EventType.MouseDown && Event.current.button == 0 )
        {
            ...
            //nv.Dev.Clamp(ref test, currentLayer.ValidArea);

            //currentLayer.FillArea( test, id_color );
            //GenerateCurrentLayerTexture();

            //currentLayer[ x, y ].id = id_color;
            //for( int j = (int)nv.Dev.RectTopLeft( test ).y; j < (int)nv.Dev.RectBottomRight( test ).y; ++j )
            //{
            //    for( int i = (int)nv.Dev.RectTopLeft( test ).x; i < (int)nv.Dev.RectBottomRight( test ).x; ++i )
            //    {
