...
public class IA_Game_DPAD_V : InputAction
{
    ...
    public override void DoAction( InputActionEventData player )
    {
        ...
        if( player.GetAxis() < 0 )
        {
            _input_interface.GetTarget().HandleInputEvent_DPad_Down( player );
        }

        if( player.GetAxis() > 0 )
        {
            _input_interface.GetTarget().HandleInputEvent_DPad_Up( player );
        }
    }
}
