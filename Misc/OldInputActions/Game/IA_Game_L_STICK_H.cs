...
public class IA_Game_L_STICK_H : InputAction
{
    ...
    public override void DoAction( InputActionEventData player )
    {
        ...
        if( player.GetAxis() < 0 )
        {
            _input_interface.GetTarget().HandleInputEvent_LStick_Left( player );
        }

        if( player.GetAxis() > 0 )
        {
            _input_interface.GetTarget().HandleInputEvent_LStick_Right( player );
        }
    }
}
