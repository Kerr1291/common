...
public partial class MapLayer
{
    ...
    public bool DataIsValid()
    {
        if( w * h != Count )
        {
            DLog.LogWarning( "Error: size " + ( w * h ) + " != data size " + Count + " for layer " + name + "; Possible data corruption." );
            return false;
        }
        return true;
    }
    ...
    {
        Utils.Clamp( ref source_area, source.Area );
        Utils.Clamp( ref dest_area, dest.Area );
    ...
    {
        return Utils.Contains(x, y, Vector2.zero, Size);
    }
