...
public class IA_Game_R_STICK_V : InputAction
{
    ...
    public override void DoAction( InputActionEventData player )
    {
        ...
        if( player.GetAxis() < 0 )
        {
            _input_interface.GetTarget().HandleInputEvent_RStick_Down( player );
        }

        if( player.GetAxis() > 0 )
        {
            _input_interface.GetTarget().HandleInputEvent_RStick_Up( player );
        }
    }
}
