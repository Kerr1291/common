...
namespace CardGame
{
    public class CardGameController : MonoBehaviour
    {
        ...
        {
            if( gameSeed != 0 )
                RNG.Instance.ResetRng( gameSeed );
            else
                RNG.Instance.ResetRng();

            Shuffle( ref cardDeck );
        }

        public static void Shuffle( ref byte[][] data )
        {
            int n = data.Length;
            while( n > 1 )
            {
                ...
                byte[] value = new byte[(int)Card.Feature.Count];
                for( int i = 0; i < (int)Card.Feature.Count; ++i )
                    value[ i ] = data[ k ][ i ];
        ...
        public void StartGame()
        {
            ...
            players[ RNG.Randi()%players.Count ].NotifyStartTurn();
        }
