...
public class IA_Game_DPAD_H : InputAction
{
    ...
    public override void DoAction( InputActionEventData player )
    {
        ...
        if( player.GetAxis() < 0 )
        {
            _input_interface.GetTarget().HandleInputEvent_DPad_Left( player );
        }

        if( player.GetAxis() > 0 )
        {
            _input_interface.GetTarget().HandleInputEvent_DPad_Right( player );
        }
    }
}
