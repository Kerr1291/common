...
public class SimpleView : MonoBehaviour
{
    ...
    public void TestGenerate() 
    {
        ...
        for( int i = 0; i < mappings.Count; ++i )
        {
            if( mappings[ i ].combineMeshes )
            {
                ...
                MeshFilter mesh = mappings[ i ].root.AddComponent<MeshFilter>();
                ...
                MeshCollider mc = mappings[ i ].root.AddComponent<MeshCollider>();
        ...
        if( player != null )
        {
            ...
            {
                player.transform.position = Utils.VectorXZ(start_spots[0].position, 2.0f);
            }            
        }
    }
    ...
    void GenerateLayer(MapLayer layer)
    {
        ...
        { 
            ...
            Vector3 p = Utils.VectorXZ(m.pos, type.prefab.transform.localPosition.y);
