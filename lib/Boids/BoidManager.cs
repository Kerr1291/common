...
namespace nv.editor
{
    ...
    [CustomEditor(typeof(Boids.BoidManager))]
    public class BoidManagerEditor : Editor
    {
        Boids.BoidManager _target;

        public override void OnInspectorGUI()
        {
            _target = (Boids.BoidManager)target;
            ...
            for(int i = 0; i < _target.keys.Count; ++i)
            {
                ...
                Boids.BoidRules value = (Boids.BoidRules)EditorGUILayout.ObjectField("BoidRules", _target.values[i], typeof(Boids.BoidRules), true);
